...
```collections.abc```模块中有```Mapping```和```MutableMapping```这两个抽象基类，它们的作用是为dict和其他类似的类型定义形式接口（在Python2.6到Python3.2的版本中，这些类还不属于```collections.abc```模块，而是隶属于```collections```模块。）
 ...
 ![](https://img-blog.csdn.net/20180828191547701?watermark/2/text/aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzQxNzY4NDAw/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70)
