...
# 模型保存的路径和文件名
MODEL_SAVE_PATH = './model/convolutional/'
MODEL_NAME = 'convolutional.ckpt'


def train(mnist):
    # 定义输入输出placeholder
    x = tf.compat.v1.placeholder(
        tf.float32, shape=[BATCH_SIZE, common.IMAGE_SIZE, common.IMAGE_SIZE, common.NUM_CHANNELS], name='x-input')
    ...
    train = True
    y,_ = common.Convolution.inference(x, train, regularizer)
    ...
    cross_entropy = tf.compat.v1.nn.sparse_softmax_cross_entropy_with_logits(logits=y, labels=tf.compat.v1.argmax(y_, 1))
    cross_entropy_mean = tf.compat.v1.reduce_mean(cross_entropy)
    loss = cross_entropy_mean + tf.compat.v1.add_n(tf.compat.v1.get_collection('losses1'))
    ...
    train_op = tf.compat.v1.group([train_step, variable_averages_op])
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for i in range(TRAINING_STEPS):
            ...
            xs_new = np.reshape(xs, (BATCH_SIZE, common.IMAGE_SIZE, common.IMAGE_SIZE, common.NUM_CHANNELS))
            _, loss_value, step, accuracy_score = sess.run([train_op, loss, global_step, accuracy], feed_dict={x: xs_new, y_: ys})
            if i % 1000 == 0:
                print(sess.run((tf.compat.v1.get_collection("losses1"))))
                # 输出当前的训练情况，这里只输出了模型在当前训练batch上的损失函数大小，通过损失函数的大小可以大概了解训练的
                #  情况，在验证数据集上的正确率信息会有一个单独的程序来生成
                print("After {} training step, loss on training batch is {}, accuracy is {}".format(i, loss_value, accuracy_score))
